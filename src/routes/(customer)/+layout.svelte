<script lang="ts">
	...
	onMount(async () => {
		console.log('Customer layout mounted - loading user...');
		await authStore.loadUser();
		console.log('User loaded:', $authStore.user);
		
		// After loading, check if user is customer
		if (!$authStore.user || $authStore.user.role !== 'customer') {
			console.log('User is not customer, redirecting to login');
			goto('/login');
			return;
		}
	...
	const navItems = [
		{ label: 'Dashboard', href: '/customer', icon: 'dashboard', exactMatch: true },
		{ label: 'Book Cargo', href: '/customer/book', icon: 'book' },
		{ label: 'Shipments', href: '/customer/shipments', icon: 'shipments' },
		{ label: 'Track Cargo', href: '/customer/track', icon: 'track' },
		{ label: 'Invoices', href: '/customer/invoices', icon: 'invoices' },
	];
...
<svelte:head>
	<title>Customer Portal - GoFright</title>
</svelte:head>
...
{:else}
	<div class="flex h-screen bg-gradient-to-br from-blue-50 via-purple-50 to-pink-50 overflow-hidden">
		<!-- Sidebar -->
		<aside class="w-[120px] bg-gradient-to-b from-orange-600 to-orange-700 flex flex-col items-center py-6 relative shadow-2xl" style="border-radius: 0 30px 30px 0;">
			<!-- Logo -->
			<div class="mb-8">
			...
			<nav class="flex-1 flex flex-col items-center space-y-6 mt-4">
				{#each navItems as item}
					<a
						href={item.href}
						class="w-11 h-11 rounded-xl flex items-center justify-center transition-all duration-200 {isActive(item.href, item.exactMatch) ? 'bg-white text-orange-600 shadow-md scale-110' : 'text-white/60 hover:text-white hover:bg-white/10'}"
						title={item.label}
					>
			...
			<div class="mt-auto pt-4">
				...
				>
					...
					{:else}
						<div class="w-full h-full bg-gradient-to-br from-orange-400 to-orange-600 flex items-center justify-center text-white font-bold text-sm">
							{getInitials($authStore.user?.name || 'CU')}
						</div>
					{/if}
				</a>
			</div>
